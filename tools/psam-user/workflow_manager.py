...
class WorkflowManager(object):
    ...
    def convertPSAlist_normal(self, userID, upr_url, psar_url):
        ...
        for psa in psa_list:
            ...
            if len(par_data.json()) == 0:
                print "ERROR: PSA not found: " + str(psa)
                return
    ...
    def convertSG(psa_list_json, mspl_psa_assoc, psar_url):
        output = """<?xml version=\"1.0\" encoding=\"UTF-8\"?>
		        <tns:Mapping xmlns:tns=\"http://www.example.org/Refinement_Schema\" 
		                     xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" 
		                     xsi:schemaLocation=\"http://www.example.org/Refinement_Schema\">
		                     <tns:service_graph>"""
        max_number = psa_list_json['length']
        ...
        for psa in psa_list_json:
            mspl_id = []
            if psa['Generic'] == "true":
                output = ' <tns:service serviceID="idvalue' + str(i-1)
                if mspl_psa_assoc.contains(psa.psaID):
                    output = ' MSPL_ID="' + mspl_psa_assoc.get(psa.psaID)[0] + '">'
                else:
                    output = '">'

                output += "<tns:capability>"
                capability_list = psar.get_psa_capabilities(psa.psaID)
                for c in capability_list:
                    output += "<tns:capability_list>" + c + "</tns:capability_list>"
                    output += "</tns:capability>"

            else:
                output += "<tns:service serviceID=\"" + idvalue + "\"" + i + " \"> "
                output += "<tns:PSA name=\" " + psa.psaID + "\">"
                output += "<tns:PSA_info />"
                opt_par = psar.get_psa_opt_par(psa.psaID)
                if opt_par.status_code != 200:
                    raise Exception("ERROR: No optimisation profile for this PSA ID: " + str(psa.psaID))

                output += "<tns:PSA_characteristic cost=\"" + opt_par.cost + "\" latency=\" " \
                          + opt_par.latency + "\" rating=\"" + opt_par.rating + "\"/>";
                ...
                if mspl_id.length > 0:
                    ...
                    for mspl_id in mspl_id_list:
                        output += " <tns:mspl_list id=\"" + mspl_id + "\"/>"
        ...
        output += " <tns:rootService>idvalue0</tns:rootService>"
        output += " <tns:endService> " + str(max_number - 1) + "</tns:endService>"

        i = 0
        while (i < max_number - 1):
            output += "<tns:edge>"
            output += " <tns:endService>idvalue " + str(i) + "</tns:endService>"
            output += " <tns:endService>idvalue " + str(i + 1) + "</tns:endService>"
    ...
    def H2M(self, user, password, upr_url, spm_url, psar_url, editor_id=None):
        ...
        if r.status_code == 404:
		    ...
		    #isa
                    ...
                    if usertype == "expert":
                        user_sg = self.convertSG(market_psa_list_xml, mspl_psa_assoc, psar_url)
                    else:
                        psa_xml = self.convertPSAlist_normal(user, upr_url, psar_url)
                    ...
                    psa_xml = base64.b64encode(psa_xml)
