...
class Client:
	...
	def create_psa(self, name=None, token=None, id=None, manifest_id=None, 
		plugin_id=None, cost=None,owner=None, latency=None, rating=None, 
		is_generic=None, psa_storage_id=None, general_info_id=None, funcionality_id=None,
		execution_model_id=None, configuration_model_id=None, monitoring_id=None,
		custom_id=None,psa_description=None ):
		...
		if token:
			data['token']=token
                ...
                if psa_storage_id:
                        data['psa_storage_id']=psa_storage_id
                if general_info_id:
                        data['general_info_id']=general_info_id
                if funcionality_id:
                        data['funcionality_id']=funcionality_id
                if execution_model_id:
                        data['execution_model_id']=execution_model_id
                if configuration_model_id:
                        data['configuration_model_id']=configuration_model_id
                if monitoring_id:
                        data['monitoring_id']=monitoring_id
                if custom_id:
                        data['custom_id']=custom_id
                if psa_description:
                        data['psa_description']=psa_description
	...
	def get_image_list(self, id=None,token=None, is_generic=None, cost=None, latency=None, rating=None, capability=None, owner=None):
		url = self.base_url+'PSA/images/'
                params={}
                if token:
                        params['token']=token
...
if __name__=='__main__':
	...
	def list_psa(args):
		if args.url:
			c=Client(args.url)
			 ...
			 c=Client(PSAR_URL)
		r=c.get_image_list(token=args.token, id=args.id, is_generic=args.is_generic, cost=args.cost, latency=args.latency, rating=args.latency,
		capability=args.capability, owner=args.owner,)
		"""
		if args.id:
	...
	def download_manifest(args):
                if args.url:
                        c=Client(args.url)
                         ...
                         c=Client(PSAR_URL)
		r=c.get_manifest(path=args.path,psa_id=args.id,token=args.token)
	...
	def create_psa(args):
                if args.url:
                        c=Client(args.url)
                         ...
                         c=Client(PSAR_URL)
		r=c.create_psa(id=args.id,name=args.name,  manifest_id=args.manifest_id,plugin_id=args.plugin_id,
			cost=args.cost, owner=args.owner, latency=args.latency, rating=args.rating,
			is_generic=args.is_generic, psa_storage_id=args.psa_storage_id, general_info_id=args.general_info_id,
			funcionality_id=args.funcionality_id, execution_model_id=args.execution_model_id,
			configuration_model_id=args.configuration_model_id, monitoring_id=args.monitoring_id, 
			custom_id=args.custom_id, psa_description=args.psa_description, )

		print r.text
	def put_dyn_conf(args):
                if args.url:
                        c=Client(args.url)
                else:
                         c=Client(PSAR_URL)
		r=c.put_dyn_conf(psa_id=args.id, location=args.location, dyn_conf=args.dyn_conf, token=args.token)		
	def get_dyn_conf(args):
                if args.url:
                        c=Client(args.url)
                else:
                         c=Client(PSAR_URL)
		r=c.get_dyn_conf(psa_id=args.id, location=args.location, token=args.token)		
	def delete_dyn_conf(args):
                if args.url:
                        c=Client(args.url)
                else:
                         c=Client(PSAR_URL)
		r=c.delete_dyn_conf(psa_id=args.id, location=args.location, token=args.token)		
	...
	create_parser.add_argument('--id',action='store', help='ID of the new PSA')
	create_parser.add_argument('--name',action='store', help='Name of the new PSA')
	create_parser.add_argument('--manifest_id',action='store', help='Manifest ID of the new PSA')
	create_parser.add_argument('--plugin_id',action='store', help='Plugin ID of the new PSA')

	create_parser.add_argument('--psa_storage_id',action='store', help='PSA storage ID of the new PSA')
	create_parser.add_argument('--general_info_id',action='store', help='General Info ID of the new PSA')
	create_parser.add_argument('--funcionality_id',action='store', help='Funcionality ID of the new PSA')
	create_parser.add_argument('--execution_model_id',action='store', help='Execution model ID of the new PSA')
	create_parser.add_argument('--configuration_model_id',action='store', help='configuration model ID of the new PSA')
	create_parser.add_argument('--monitoring_id',action='store', help='Monitoring ID of the new PSA')
	create_parser.add_argument('--custom_id',action='store', help='Custom ID of the new PSA')
	create_parser.add_argument('--psa_description',action='store', help='PSA description of the new PSA')

	create_parser.add_argument('--cost',action='store', help='Cost of the new PSA')
	create_parser.add_argument('--owner',action='store', help='Owner of the new PSA')
	create_parser.add_argument('--latency',action='store', help='Latency of the new PSA')
	create_parser.add_argument('--rating',action='store', help='Rating of the new PSA')
	create_parser.add_argument('--is_generic',action='store', help='param is generic of the new PSA')
	...
	list_parser = subparsers.add_parser('list', help='List PSAs stored')
        ...
        list_parser.add_argument('--cost', action='store', help='Returns only matching PSAs')
        list_parser.add_argument('--latency', action='store', help='Returns only matching PSAs')
        list_parser.add_argument('--rating', action='store', help='Returns only matching PSAs')
        list_parser.add_argument('--owner', action='store', help='Returns only matching PSAs')
        list_parser.add_argument('--is_generic', action='store', help='Returns only matching PSAs')
        list_parser.add_argument('--capability', action='store', help='Returns only matching PSAs')
	...
	#Capabilities
        capability_parser=subparsers.add_parser('capabilities', help='Interacts with the capabilities')
	capability_subparser=capability_parser.add_subparsers(help='commands')
